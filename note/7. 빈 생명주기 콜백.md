...
너무 당연한 이야기지만 객체를 생성하는 단계에는 url이 없고, 객체를 생성한 다음에 외부에서 수정자 주입을 통해 `setUrl()`이 호출되어야 url이 존재하게 된다.
...
> 🍀 참고: 싱글토 빈들은 스프링 컨테이너가 종료될 때 싱클톤 빈들도 함께 종료되기 때문에 스프링 컨테이너가 종료되기 직전에 소멸전 콜백이 일어난다.
...
> 🍀 참고: 인터페이스를 사용하지 않는 초기화, 종료 방법은 스프링 초창기에 나온 방법이고, 지금은 더 좋은 방법이 있기에 거의 사용하지 않는다.
...
- 이 추론 기능은 `close`, `shutdown` 이름의 메서드를 자동으로 호출한다. 이름 그대로 종료 메서드를 추론해서 호출해준다.
...
- 패키지를 잘 보면 `javax.annotation.PostConstruct`이다. 스프링에 종속적인 기술이 아니라 `JSR-250` 자바 표준이다. 따라서 스프링이 아닌 다른 컨테이너에서도 동작한다.
- 컴포넌트 스캔과 잘 어울린다.
